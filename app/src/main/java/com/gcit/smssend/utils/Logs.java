package com.gcit.smssend.utils;
...
public class Logs {
    ...
    public static void init(boolean logSwitch, boolean log2FileSwitch, char logFilter, String tag) {
        if (log2FileSwitch && Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            dir = Utils.getApp().getExternalCacheDir().getPath() + File.separator;
        } else {
            dir = Utils.getApp().getCacheDir().getPath() + File.separator;
        }
        Logs.logSwitch = logSwitch;
        Logs.log2FileSwitch = log2FileSwitch;
        Logs.logFilter = logFilter;
        Logs.tag = tag;
        Logs.mGson = new Gson();
    }
    ...
    public static Builder getBuilder() {
        if (log2FileSwitch && Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            dir = Utils.getApp().getExternalCacheDir().getPath() + File.separator + "log" + File.separator;
        } else {
            dir = Utils.getApp().getCacheDir().getPath() + File.separator + "log" + File.separator;
        }
        return new Builder();
    }
...
//        return String.format(format, callerClazzName);
        return new StringBuilder().append(Logs.tag).append("[").append(tag).append("] ").toString();
    }
